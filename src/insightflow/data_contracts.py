...
def _issue(
    severity: str, rule_id: str, column: str, failed_rows: int, message: str
) -> ContractIssue:
...
def validate_contract(df: pd.DataFrame, contract: dict[str, Any]) -> pd.DataFrame:
    ...
    for column, rule in contract.get("rules", {}).items():
        ...
        if "min" in rule or "min_exclusive" in rule or "max" in rule:
            ...
            if "min" in rule:
                ...
                if count:
                    issues.append(
                        _issue("P1", "min_value", column, count, f"{column}低于最小值{rule['min']}")
                    )
                ...
                if count:
                    issues.append(
                        _issue(
                            ...
                            column,
                            count,
                            f"{column}不大于{rule['min_exclusive']}",
                        )
                    )
            if "max" in rule:
                ...
                if count:
                    issues.append(
                        _issue("P1", "max_value", column, count, f"{column}高于最大值{rule['max']}")
                    )
        if rule.get("type") == "datetime":
            ...
            if count:
                issues.append(
                    _issue("P1", "datetime_type", column, count, f"{column}无法解析为日期")
                )
            if rule.get("not_future"):
                ...
                if count:
                    issues.append(
                        _issue("P1", "not_future", column, count, f"{column}包含未来日期")
                    )
    ...
    if key and all(column in df.columns for column in key):
        ...
        if count:
            issues.append(
                _issue("P1", "primary_key_unique", ",".join(key), count, "业务主键存在重复")
            )
...
def contract_block_message(issues: pd.DataFrame) -> str:
    ...
    if blocking.empty:
        return ""
    details = "; ".join(
        f"{row.column}/{row.rule_id}: {int(row.failed_rows)} rows" for row in blocking.itertuples()
    )
