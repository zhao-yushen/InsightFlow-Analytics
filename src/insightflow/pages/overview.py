...
def _metric(
    ...
    *,
    ...
    st.metric(label, value, "样本不足" if not comparison_enabled else ("—" if delta is None else f"{delta:+.1%}"))
...
def render() -> None:
    ...
    if int(current_coverage["orders"]) == 0:
        st.warning(
            lt("当前筛选组合没有有效销售数据。请清空某个维度、扩大日期范围，或检查市场区域与国家是否匹配。")
        )
    ...
    if not comparison_enabled:
        st.warning(
            lt("当前或上一比较周期的订单少于5笔。系统保留绝对值，但暂停百分比变化与异常结论，避免小样本误导。")
        )

    section_header(lt("核心经营指标"), lt("相较上一等长周期"))
    top = st.columns(5)
    with top[0]:
        _metric(lt("净销售额"), f"£{current_kpi['revenue']:,.0f}", current_kpi["revenue"], previous_kpi["revenue"], comparison_enabled=comparison_enabled)
    with top[1]:
        _metric(
            "贡献利润",
    ...
    with top[3]:
        _metric(lt("订单量"), f"{current_kpi['orders']:,.0f}", current_kpi["orders"], previous_kpi["orders"], comparison_enabled=comparison_enabled)
    with top[4]:
        _metric(
            "活跃客户",
    ...
    with secondary[0]:
        _metric(lt("毛利率"), f"{current_kpi['gross_margin']:.1%}", current_kpi["gross_margin"], previous_kpi["gross_margin"], comparison_enabled=comparison_enabled)
    with secondary[1]:
        _metric(lt("复购率"), f"{current_kpi['repeat_rate']:.1%}", current_kpi["repeat_rate"], previous_kpi["repeat_rate"], comparison_enabled=comparison_enabled)
    with secondary[2]:
    ...
    with right:
        with st.container(border=True):
            ...
            issues, _ = generate_diagnostics(DEFAULT_DB_PATH, filters) if comparison_enabled else ([], pd.DataFrame())
            if not issues:
    ...
    with lower[1]:
        with st.container(border=True):
            ...
            if targets.empty:
                ...
                shown_targets["attainment"] = shown_targets["attainment"].map(lambda value: f"{value:.1%}")
