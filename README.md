...
> **v0.4.3.3 筛选可靠性与客户留存修复：** 细分维度改为区域→国家→品类→渠道的级联联动，并显示当前命中样本；0订单和少于5笔订单时不再生成误导性的百分比变化；修复客户队列留存报错；行动状态改为“未闭环”口径；数据接入中心新增全量替换模式。

> **v0.4.2.1 修复：** 已为每个 Streamlit 页面设置唯一 URL，解决多页面均被推断为 `/render` 的启动错误。
...
- **可复现实验**：Bootstrap随机种子改用SHA-256稳定哈希，跨进程运行保持一致。
- **行动持久化**：稳定问题指纹去重，负责人、截止日期、状态和处理备注写回SQLite。
- **真正只读模式**：访客不能修改行动、运行故障演练或向服务器磁盘生成报告。
- **内存报告**：HTML与Word报告可直接在内存生成并下载，适合只读公开部署。
- **查询缓存**：按SQLite数据库与WAL版本缓存只读查询，数据更新后自动失效。
- **可安装CLI**：提供 `insightflow bootstrap/validate/report/benchmark/run` 统一命令。
- **部署加固**：非root Docker、健康检查、只读根文件系统、能力降权和资源限制。
- **测试增强**：33项测试，核心后端覆盖率83.8%。
- **数据可信度标签**：交易、成本和库存分别标注 Verified、Estimated、Simulated 或 Mixed。
- **指标信任元数据**：每个指标记录定义、负责人、数据状态、置信度和来源说明。
- **正式数据契约**：检查必要字段、业务主键、日期、数值范围、枚举和未来日期。
- **故障注入演练**：在内存中注入重复、缺失、负价格、未来日期、未知渠道和缺失月份，验证质量体系是否有效。
- **双数据结构**：包含单表演示数据、七表电商演示结构，并提供公共 Olist 多表数据适配器。
- **概率决策实验室**：在点估计之外运行 Monte Carlo 模拟，输出利润改善概率、90%区间和“增收但减利”概率。
- **A/B实验中心**：计算转化、收入、利润、退货护栏与Bootstrap区间，利润受损时阻止全量推广。
- **经营行动中心**：将异常、目标和库存风险转为证据化行动清单，附负责人和截止日期。
- **预测评价增强**：增加 WAPE、sMAPE、MASE 和 Bias，不再只依赖 MAPE。
- **Power BI 模型包**：导出事实表、维度表、实验表和治理表，并提供DAX、模型图和页面蓝图。
- **招聘展示材料**：90秒演示脚本、利润下降案例、招聘作品集说明和性能基准。
